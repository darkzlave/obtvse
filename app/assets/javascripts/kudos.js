...
function sendKudoInfoToServer(element_id,number){
	...
	if (! sending_kudo){
		// /<slug>/kudos.xml
		var url = "/" + element_id + "/kudos.xml";
		$.post(url,{kudos: number});
		sending_kudo = true;
...
function startKudoing(element) {
	...
	setTimeout("sendKudo('" + element_id +"',1)", 700 );
}
...
// whatever it is.
function sendKudo(element_id,number) {
	var element = $("#"+element_id);
	element.flag = true; 
	element.article = element.closest("article").attr("id");
	sendKudoInfoToServer(element_id,number);
